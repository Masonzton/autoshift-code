...
void configure_gclock_generator(void)
{
	...
	system_gclk_gen_set_config(GCLK_GENERATOR_8, &gclock_gen_conf);
	system_gclk_gen_enable(GCLK_GENERATOR_8);
}
...
int main(void) {
	...
	display_menu();
	configure_can();
	can_set_baudrate(CAN_MODULE, 38400);
	print("it worked");
	uint16_t key;
	while(true){
		if (usart_read_wait(&com_instance, &key) == STATUS_OK) {
			...
			//delay_ms(500);
			uint8_t message[] = {0,1,2,3,65,65,65,65};
			uint8_t message1[] = {0,1,2,3,4,5,6,7};
			uint8_t message2[] = {8,9,10,11,12,13,14,15};
			//uint8_t message3[] = {0x24,0x24,0x24,0x24,0x20,0x20,0x20,0x20};			
			can_send_standard_message(CAN_RX_STANDARD_FILTER_ID_1, message);
			delay_ms(500);
			can_send_standard_message(CAN_RX_STANDARD_FILTER_ID_1, message1);
			delay_ms(500);
			can_send_standard_message(CAN_RX_STANDARD_FILTER_ID_1, message2);
			//can_send_standard_message(CAN_RX_STANDARD_FILTER_ID_1, message3);
				break;
				default:
				break;
			}
